          ...
          link: function(scope, elem, attr, API) {
            ...
            scope.quizSubmit = function() {
              if (vgQuizSubmitCallBack) {
                var paramObj = {};
                angular.forEach(elem.find('form').find('input'), function(value, key) {
                  if (paramObj.hasOwnProperty(value.name)) {
                    if (!paramObj[value.name].push) {
                      paramObj[value.name] = [paramObj[value.name]];
                    }
